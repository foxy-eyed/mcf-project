...
- [ADR: реализация контекста скоринга кандидатов в виде отдельного сервиса](#adr-реализация-контекста-скоринга-кандидатов-в-виде-отдельного-сервиса)
...
Оставшиеся контексты можно объединить в один сервис (обозначим его Core-монолит), поскольку:
...
|-----------------------------|----------------------------------------------------------------------------|----------------|
| Скоринг                     | Layered monolith                                                           | Реляционная БД |
| Матчинг                     | Pipeline                                                                   | Реляционная БД |
| Списание средств с клиентов | Microkernel<br>(работа с платежными шлюзами может быть вынесена в плагины) | Реляционная БД |
| Комплектация заказов        | Layered monolith                                                           | Реляционная БД |
| Core-монолит                | Modular monolith                                                           | Реляционная БД |

Каждый из сервисов имеет собственную реляционную БД из-за требования CatFinCompliance на уровне всей системы.

### Коммуникации

Сервис **Скоринга** после апрува кандидата должен отправлять стриминговое событие сразу в два сервиса:
в **Матчинг** и в **Core-монолит**. Можно делать два синхронных запроса, но поскольку нам не нужно ничего получать в ответ, 
мы можем выбрать асинхронную event-driven коммуникацию: генерировать событие для произвольного кол-ва консьюмеров.

**Core-монолит** должен вызывать сервис **Матчинга** сразу после создания заказа. Обращение к матчеру имеет смысл сделать 
...
Для консистентности event-driven коммуникацию распростанила на все взаимодействия Core-монолита с другими сервисами.
...
|-----------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------|
| Вся система                 | - Проверка частоты релизов (не реже раза в месяц);<br>- Покрытие тестами не ниже X (CI pipeline);<br>- Сокрость доставки фич в прод (метрики в Jira); |
...
| Core-монолит                | - FF для контроля кросс-контекстных вызовов (допустим, у нас проект на Ruby :))                                                                       |

## ADR: реализация контекста скоринга кандидатов в виде отдельного сервиса
### Status
Proposed
...
Для реализации сервиса преложен Layered архитектурный стиль.
...
- Сокрость доставки фич в прод (метрики в Jira);
- Нагрузочные тесты;
